...
class TaskDeadlinePressure(BaseTradeTask):
    ...
    def get_market_narrative(
        ...
        if completion_pct >= 99.9:
            gate_status = "🏆 COMPLETION GATE: CLEARED ✅ — IS quality now determines your score!"
            ...
            gate_status = (
                ...
                f"Maintain rate ≥ 0.05 and do NOT slow down."
            )
        else:
            shortfall = shares_remaining - int(pace_needed * steps_left)
            gate_status = (
                f"🚨 COMPLETION AT RISK: Need {pace_needed:,.0f}/step but current pace is insufficient. "
                f"Shortfall: {shortfall:,} shares. SCORE WILL BE 0.0 IF YOU MISS THE DEADLINE."
            )
        ...
        if steps_left <= 10 and shares_remaining > 1_000:
            urgency = (
                ...
                f"Set participation_rate=0.25 (MAX) immediately. IS score is secondary now."
            )
        elif steps_left <= int(self.max_steps * 0.25) and shares_remaining > 0:
            urgency = (
                f"\n⚠️  FINAL QUARTER: {shares_remaining:,} shares left in {steps_left} steps "
                f"({pace_needed:,.0f}/step needed). Increase aggression — completion > IS quality."
            )
        elif step_count < int(self.max_steps * 0.20):
            urgency = (
                f"\n💡 OPEN STRATEGY: Front-load now (rate 0.10–0.15). "
                f"1M shares in 80 steps requires {required_pace:,.0f}/step average — start fast!"
            )
        else:
            urgency = ""
        ...
        if current_is == 0.0:
            ...
            is_hint = f" IS={current_is:.1f} bps (focus on fills first, IS second)."
        else:
            is_hint = f" IS={current_is:.1f} bps."
